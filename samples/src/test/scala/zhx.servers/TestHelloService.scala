...
object ExampleSpec extends DefaultRunnableSpec(
  suite("routes suite")(
    ...
    testM("unmapped request returns not found") {
      val io: Task[Response[Task]] = Hello1Service.service.run(Request[Task](Method.GET, Uri.uri("/a")))
      io.fold(
        e => fail(Cause.fail(e)),
    ...
    testM("root request body returns hello!") {
      (for{
        request <- Hello1Service.service.run(Request[Task](Method.GET, Uri.uri("/")))
